...
Current state: **Aufgaben 0–8 done (all course tasks)** — Rails 8.1.3.1 app in the repo root; all models with validations, unique indexes and German messages (`config/locales/de.yml`); seeds, fixtures, model tests. Authentication complete (F1): `Authentication` concern (`current_user` helper works in public actions too, `return_to` after login, locked accounts rejected), `SessionsController`, `RegistrationsController` (`/signup`), `PasswordsController`, German views, navigation in the layout, `docs/sicherheit.md`. Profile complete: `ProfilesController` (show/edit/update, always `Current.user`, no ID in any route), `PasswordChangesController` (current password required, other sessions terminated), `EmailChangesController` + `EmailConfirmationsController` (`unconfirmed_email` plus `generates_token_for :email_confirmation`, mail sent after commit, link logged — no SMTP), `UserMailer`. Authorization started (F1/Q2): Pundit installed, `ApplicationPolicy` (guests handled, default deny), `UserPolicy` (admin only; self-lock/self-delete/self-demotion blocked via `permitted_attributes`), `Admin::BaseController` (`after_action :verify_authorized`) + `Admin::UsersController` (index/edit/update/lock/unlock/destroy), `rescue_from` for 403 and 404 with `app/views/errors/`. Authorization complete (Q2): `ProductPolicy`, `RatingPolicy`, `ReportPolicy`, `ActivityPolicy` (headless) with scopes, `verify_authorized`/`verify_policy_scoped` global in `ApplicationController` — controllers without a foreign record opt out via the `SkipAuthorization` concern; permission matrix in `docs/projektantrag.md` 4.3. Core function complete (F2–F8): `ProductsController` (search/filter, detail, create with duplicate handling, moderator edit with optimistic locking, lock/unlock), `RatingsController` (`Rating.with_aggregates` wraps every change in one transaction — Q1), `ReportsController` + `Moderation::ReportsController` (pessimistic `claim!`), views for Screens 1 and 3–9, `root "products#index"`. All Q3 conflicts documented in `docs/fehlerbehandlung.md`. Activity log done: PaperTrail on `Product` (aggregates ignored), `Rating`, `Report`; `whodunnit` via `user_for_paper_trail`; `/activities` feed and a per-product history for moderators (`ActivityPolicy`). `config.active_record.yaml_column_permitted_classes` is required — without it `version.changeset` silently returns `{}`. Testing done: 240 tests, 4.0 s, no skips; `docs/testing.md` maps every F/Q requirement to its test, `script/search_benchmark.rb` measures Q4 (product list is paginated, `ProductsController::PER_PAGE`, otherwise Q4 fails at 5000 products). Static error pages in `public/` are German and themed; glossary is `projektantrag.md` section 7. UI is themed after the game Outer Wilds (`application.css`; background effects toggled by the only custom JS in `application.js`), product cards show category illustrations (`ProductsHelper#category_illustration`), products averaging ≥ 4 stars get the «Probiert» seal (`Product#certified?`). Remaining: PDF export and slides for submission (see `docs/projektplan.md`, Abschluss). The implementation plan is `docs/projektplan.md` (Aufgabe 1–8, in course order) — local only: it and `docs/bewertung.md` are gitignored and not part of the submitted repo, so never link to them from tracked files; update its progress as tasks complete. Other key files:
