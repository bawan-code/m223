...
Current state: **Aufgaben 0–3 done** — Rails 8.1.3.1 app in the repo root; all models with validations, unique indexes and German messages (`config/locales/de.yml`); seeds, fixtures, model tests. Authentication complete (F1): `Authentication` concern (`current_user` helper works in public actions too, `return_to` after login, locked accounts rejected), `SessionsController`, `RegistrationsController` (`/signup`), `PasswordsController`, German views, navigation in the layout, `docs/sicherheit.md`. Profile complete: `ProfilesController` (show/edit/update, always `Current.user`, no ID in any route), `PasswordChangesController` (current password required, other sessions terminated), `EmailChangesController` + `EmailConfirmationsController` (`unconfirmed_email` plus `generates_token_for :email_confirmation`, mail sent after commit, link logged — no SMTP), `UserMailer`. Next is Aufgabe 4 (Pundit, `Admin::UsersController`). Placeholder `pages#home` root route until Aufgabe 6. The implementation plan is `docs/projektplan.md` (Aufgabe 1–8, in course order); update its progress as tasks complete. Other key files:
